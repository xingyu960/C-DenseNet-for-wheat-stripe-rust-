...
from utils.simplemetrics import precision,recall,f1score,accuracy
...
def accuracy(output, labels, topk=(1,)):
    ...
    for j in range(pred.size(0)):
        # txt_file = open("/home/mzw/python/pytorch/classification1/dataset/train4/test/label.txt", "a",
        #                 encoding="utf-8")  # 以写的格式打开先打开文件
        # txt_file.write(str(labels[j]))
        # txt_file.write("\n")
        # txt_file.close()
        # txt_file = open("/home/mzw/python/pytorch/classification1/dataset/train4/test/out.txt", "a",
        #                 encoding="utf-8")  # 以写的格式打开先打开文件
        # txt_file.write(str(pred[j]))
        # txt_file.write("\n")
        # txt_file.close()
...
# 训练函数
def train(train_loader, model, criterion, optimizer, exp_lr_scheduler, epoch, print_interval,filename):
    # 记录数据
...
# 验证函数
def test(val_loader, model, criterion, print_interval,filename):
    # 记录数据
    batch_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
    ...
    return acc.avg, losses.avg, lab, out
