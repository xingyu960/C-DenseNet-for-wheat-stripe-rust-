...
from models.cbam_module import CBAM_Module


class _DenseLayer(nn.Sequential):
    def __init__(self, in_channels, growth_rate, bn_size):
        super(_DenseLayer, self).__init__()
        #self.add_module("CBAM_Module", CBAM_Module(in_planes=in_channels))
    ...
    def __init__(self, growth_rate=12, block_config=(6, 12, 24, 16),
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                ...
                self.features.add_module("CBAM_Module_%db" % (i + 1), CBAM_Module(in_planes=num_feature))
...
def cbam_densenet121():
    return DenseNet_BC(growth_rate=32, block_config=(6, 12, 24, 16), num_classes=6)


def cbam_densenet169():
    return DenseNet_BC(growth_rate=32, block_config=(6, 12, 32, 32), num_classes=6)


def cbam_densenet201():
    return DenseNet_BC(growth_rate=32, block_config=(6, 12, 48, 32), num_classes=6)


if __name__ == "__main__":
    net = cbam_densenet121()
    print(net)